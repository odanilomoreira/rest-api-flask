...
class Hoteis(Resource):
    def get(self):
        ...
        dados_validos = {k:dados[k] for k in dados if dados[k] is not None}

        parametros = normalize_path_params(**dados_validos)

        if not parametros.get('cidade'):
            tupla = tuple([parametros[k] for k in parametros])
...
class Hotel(Resource):
    ...
    atributos.add_argument('site_id', type=int, required=True, help='Every hotel needs a site id.')
    ...
    def post(self, hotel_id):
        ...
        hotel = HotelModel(hotel_id, **dados) # já será incluso automaticamente
        try:
            hotel.save_hotel()
        except:
